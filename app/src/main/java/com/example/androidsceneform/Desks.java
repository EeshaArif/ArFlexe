...
public class  Desks extends AppCompatActivity implements View.OnClickListener {
    private TransformableNode nameView;

    ArFragment arFragment;
    //Every 3D model has a renderable(model maker)
    private ModelRenderable desk1Renderable,
            desk2Renderable,
            desk3Renderable;


    //Images displayed for choosing the 3D model
    ImageView back, desk1, desk2, desk3;
    View arrayView[];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        desk2 = (ImageView) findViewById(R.id.desk2);
        desk3 = (ImageView) findViewById(R.id.desk3);
    ...
    private void setupModel() {
        ...
        ModelRenderable.builder()
                .setSource(this,R.raw.desk1)
                .build().thenAccept(renderable -> desk1Renderable =renderable)
                .exceptionally(
                        throwable -> {
                            Toast.makeText(this,"unable to load desk1 model",Toast.LENGTH_SHORT).show();

                            return null;});

        ModelRenderable.builder()
                .setSource(this,R.raw.desk2)
                .build().thenAccept(renderable -> desk2Renderable =renderable)
                .exceptionally(
                        throwable -> {
                            Toast.makeText(this,"unable to load desk2 model",Toast.LENGTH_SHORT).show();
        ...
        ModelRenderable.builder()
                ...
                .exceptionally(
                        throwable -> {
                            Toast.makeText(this,"unable to load desk3 model",Toast.LENGTH_SHORT).show();
    ...
    private void createModel(AnchorNode anchornode, int selected) {
        if (selected==1){
            TransformableNode desk1=new TransformableNode(arFragment.getTransformationSystem());
            desk1.setParent(anchornode);

            desk1.getScaleController().setMaxScale(25f);
            desk1.getScaleController().setMinScale(20f);
            desk1.setRenderable(desk1Renderable);
            desk1.select();
            ...
            addName(anchornode,desk1,"Natural Rustic Grren Aviator Bed Side Table\n" +
                    ...
                    "\n" +
                    "Usage/Application: Bedside table\n" +
                    "\n" +
                    "Color: Natural\n" +
                    "\n" +
                    "Brand: Rustic Grren\n" +
                    "\n" +
                    "Size: 60X60X50 (LXWXH) in cm\n" +
                    "\n" +
                    "Warranty: NA");

        }
        if (selected==2){
            TransformableNode desk2=new TransformableNode(arFragment.getTransformationSystem());
            desk2.setParent(anchornode);
            desk2.getScaleController().setMaxScale(25f);
            desk2.getScaleController().setMinScale(20f);
            desk2.setRenderable(desk2Renderable);
            desk2.select();
        ...
        if (selected==3){
            TransformableNode desk3=new TransformableNode(arFragment.getTransformationSystem());
            desk3.setParent(anchornode);
            desk3.getScaleController().setMaxScale(25f);
            desk3.getScaleController().setMinScale(20f);
            desk3.setRenderable(desk3Renderable);
            desk3.select();
    ...
    public void onClick(View view/*The view that was clicked*/) {
        ...
        else if(view.getId()==R.id.desk2){
            selected=2;
            setBackground(view.getId());
        }
        else if(view.getId()==R.id.desk3){
            selected=3;
            setBackground(view.getId());
        }
